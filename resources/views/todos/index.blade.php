        ...
        <li class=" mt-4" id="{{ $loop->iteration }}">
            <div class="flex justify-start items-center h-12 shadow drop-shadow-md rounded-[7px] px-3 hover:shadow-md">
                <div class="grow flex justify-start items-center">
                    <form method="POST" action="{{ route('todo.state',$todo) }}">
                        @csrf
                        @method('PATCH')

                        <a id="check{{ $loop->iteration }}"
                            href="{{ route('todo.state',$todo) }}"
                            @class([
                                ...
                                'green' => $todo->is_completed == 1 ? true : false,
                            ])
                            onclick="event.preventDefault();  this.closest('form').submit();">
                            <i class="text-gray-300 fa fa-check"></i>
                        </a>
                    </form>
                    <strike id="strike{{ $loop->iteration }}" 
                    @class([
                    'text-sm ml-4 text-[#5b7a9d] font-semibold',
                    'strike_none' => $todo->is_completed == 0 ? true : false,
                    ])>{{ $todo->title}}</strike>
